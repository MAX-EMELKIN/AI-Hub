// =============================================================================
// Сервис: Семейство Gemini (DoH / Proxy) (Автономный мост к AI Hub)
// ID: 700
// =============================================================================

var LOCAL_URL    = "http://127.0.0.1:8080";
var SERVICE_ID   = 700;
var SERVICE_NAME = "Gemini Family (AI Hub)";
var _IS_ALIVE    = false;
...
function serviceHeader() {
    return new ServiceHeader(
        SERVICE_ID,
        SERVICE_NAME,
        "Google Gemini Family (Flash / Pro / Lite) via QTranslate AI Hub" + Const.NL2 + LOCAL_URL,
...
function serviceTranslateRequest(text, from, to) {
    try {
        ...
        return new RequestData(HttpMethod.POST, "/gemini_family", stringifyJSON(payload), headers, CodePage.UTF8);
    } catch (e) {
...
function serviceTranslateResponse(original, json, from, to) {
    try {
        if (!json) return new ResponseData(original + Const.NL2 + "[Gemini: Сервер AI Hub не ответил]", from, to);
        var data = parseJSON(json);
        if (!data) return new ResponseData(original + Const.NL2 + "[Ошибка парсера JSON]", from, to);
        if (data.error) return new ResponseData(original + Const.NL2 + "[Ошибка Gemini: " + data.error + "]", from, to);
