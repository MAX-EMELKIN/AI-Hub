// =============================================================================
// Сервис: ChatGPT 4o Web (Автономный мост к AI Hub)
// ID: 681
// =============================================================================

var LOCAL_URL    = "http://127.0.0.1:8080";
var SERVICE_ID   = 681;
var SERVICE_NAME = "ChatGPT 4o (Web)";
var _IS_ALIVE    = false;
...
function serviceHeader() {
    return new ServiceHeader(
        SERVICE_ID,
        SERVICE_NAME,
        "ChatGPT 4o Web via Supermium / AI Hub" + Const.NL2 + LOCAL_URL,
...
function serviceTranslateRequest(text, from, to) {
    try {
        ...
        return new RequestData(HttpMethod.POST, "/chatgpt_web", stringifyJSON(payload), headers, CodePage.UTF8);
    } catch (e) {
...
function serviceTranslateResponse(original, json, from, to) {
    try {
        if (!json) return new ResponseData(original + Const.NL2 + "[ChatGPT Web: Сервер AI Hub не ответил]", from, to);
        var data = parseJSON(json);
        if (!data) return new ResponseData(original + Const.NL2 + "[Ошибка парсера JSON]", from, to);
        if (data.error) return new ResponseData(original + Const.NL2 + "[Ошибка ChatGPT Web: " + data.error + "]", from, to);
