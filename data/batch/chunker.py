# -*- coding: utf-8 -*-
"""
Модуль: data/batch/chunker.py
Назначение: Интеллектуальное разбиение файлов со 100% сохранением структуры строк (\n) и абзацев.
...
class TextChunker:
    """Инструмент для безопасного чтения, нарезки и сборки текстовых файлов."""

    def __init__(self, default_chunk_size=2500):
        self.default_chunk_size = default_chunk_size

    def read_file_safe(self, file_path):
        """Читает текстовый файл с автоматическим подбором правильной кодировки."""
    ...
    def _split_long_paragraph(self, paragraph, max_size):
        """
        Дробит длинный блок СТРОГО с сохранением переносов строк \n!
        """
        # 1. Если внутри есть переносы строк (список/чейнджлог) — делим строго по строкам \n
        if '\n' in paragraph:
        ...
        # 2. Если это один сплошной текст без переносов — делим по предложениям
    ...
    def split_into_chunks(self, text, chunk_size=None):
        """
        Разбивает текст на порции без уничтожения структуры переносов строк.
        """
