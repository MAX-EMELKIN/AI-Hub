...
POPULAR_VOICES = {
    "ru": [
        ("ru-RU-SvetlanaNeural", "Светлана (Женский, Нейро)"),
        ("ru-RU-DmitryNeural", "Дмитрий (Мужской, Нейро)")
    ],
    "en": [
        ("en-US-JennyNeural", "Jenny (Женский, США)"),
        ("en-US-GuyNeural", "Guy (Мужской, США)"),
        ("en-GB-SoniaNeural", "Sonia (Женский, Англия)")
    ],
    "de": [
    ...
    def stop_speech(self):
        with self._lock:
            try:
                ...
                logger.system("TTS: Воспроизведение речи остановлено")
    ...
    def generate_edge_tts_mp3(self, text, voice="ru-RU-SvetlanaNeural", speed=1.0):
        ...
        try:
            ...
            if " 101 " not in resp:
                ...
                raise Exception(f"Microsoft Edge TTS сбой подключения: {err_line}")
            ...
            logger.system(f"TTS: Аудио успешно получено за {elapsed:.2f}с ({len(audio_bytes)} байт)")
