# -*- coding: utf-8 -*-
"""
Модуль: data/core/logger.py
Назначение: Модульная система логирования и управления интерактивной консолью Windows.
            Поддерживает раздельное включение категорий логов, запись в файл hub_debug.log,
            маскирование секретных API-ключей, прямое обращение к WriteConsoleW для
            полной совместимости с Windows 7 (обход бага WinError 31 при CP 65001)
            и динамическое открытие/скрытие окна консоли.
...
class HubLogger:
    ...
    def _write_console_native(self, text):
        """Безопасный вывод Unicode через WinAPI WriteConsoleW (не дает сбоев на Win7)."""
        try:
    ...
    def alloc_console(self):
        ...
        if ok:
            ...
            # Устанавливаем UTF-8 кодовую страницу только на Windows 10+
            # В Windows 7 CP 65001 ломает стандартные файловые дескрипторы CRT (WinError 31)
            try:
            ...
            # Безопасный вывод шапки без риска уронить процесс
            try:
    ...
    def _write_entry(self, level_tag, service_name, content):
        ...
        # 1. Вывод в окно консоли с двойной защитой от WinError 31
        try:
            print(formatted, flush=True)
        except Exception:
            self._write_console_native(formatted + "\n")

        # 2. Запись в файл hub_debug.log
        try:
            from data.core.config_manager import config
