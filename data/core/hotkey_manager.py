...
class GlobalHotkeyManager:
    ...
    def reload_from_config(self):
        ...
        if has_any and not self.hook_handle:
            self._attach_keyboard_hook()
        elif not has_any and self.hook_handle:
            self._detach_keyboard_hook()

    def _attach_keyboard_hook(self):
        hinst = kernel32.GetModuleHandleW(None)
        if not self.hook_handle:
            self.hook_handle = user32.SetWindowsHookExW(WH_KEYBOARD_LL, self._hook_ref, hinst, 0)

    def _detach_keyboard_hook(self):
        if self.hook_handle:
            user32.UnhookWindowsHookEx(self.hook_handle)
            self.hook_handle = None

    def _attach_mouse_hook(self):
        hinst = kernel32.GetModuleHandleW(None)
        if not self.mouse_hook_handle:
            self.mouse_hook_handle = user32.SetWindowsHookExW(WH_MOUSE_LL, self._mouse_hook_ref, hinst, 0)

    def _detach_mouse_hook(self):
    ...
    def install_hook(self, main_window):
        ...
        self._attach_mouse_hook()
        self.reload_from_config()

    def uninstall_hook(self):
        self._detach_keyboard_hook()
        self._detach_mouse_hook()

    def _matches_combo(self, parsed, vk, ctrl, alt, shift, win):
        if not parsed or parsed[0] != "combo":
            return False
    ...
    def _mouse_hook_callback(self, nCode, wParam, lParam):
        if nCode == 0 and wParam == WM_LBUTTONDOWN:
            try:
                ...
                if hwnd_console and user32.IsWindowVisible(hwnd_console):
                    ms = ctypes.cast(lParam, ctypes.POINTER(MSLLHOOKSTRUCT)).contents
                    pt = ms.pt
                    rc = wintypes.RECT()
                    if user32.GetWindowRect(hwnd_console, ctypes.byref(rc)):
                        if rc.left <= pt.x <= rc.right and rc.top <= pt.y <= rc.bottom:
                            lparam_pos = ((pt.y & 0xFFFF) << 16) | (pt.x & 0xFFFF)
                            hit = user32.SendMessageW(hwnd_console, WM_NCHITTEST, 0, lparam_pos)
                            if hit == HTCLOSE:
                                user32.ShowWindow(hwnd_console, SW_HIDE)
                                try:
                                    from data.core.config_manager import config
                                    config.set_value("LOGGING", "showconsole", "0")
                                except Exception:
                                    pass
                                return 1
            except Exception:
                pass
        return user32.CallNextHookEx(self.mouse_hook_handle, nCode, wParam, lParam)
        ...
        if nCode == 0 and wParam in (WM_KEYDOWN, WM_SYSKEYDOWN):
            try:
                ...
                if vk == 0x73 and alt_down:
                    hwnd_console = kernel32.GetConsoleWindow()
                    if hwnd_console and user32.GetForegroundWindow() == hwnd_console:
                        user32.ShowWindow(hwnd_console, SW_HIDE)
                        try:
                            from data.core.config_manager import config
                            config.set_value("LOGGING", "showconsole", "0")
