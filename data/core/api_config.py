...
Назначение: Ядро конфигурации моделей и ключей. Автоматически разделяет старый api_keys.ini
            на два независимых файла: providers.ini (агрегаторы, ключи, прокси) и 
            models.ini (конкретные модели, температура, эндпоинты).
            Обеспечивает сквозное наследование ключей для всех моделей одного агрегатора.
...
# Ключи, которые принадлежат провайдеру (агрегатору), а не конкретной модели
PROVIDER_KEYS = {
...
class APIConfigManager:
    ...
    def _guess_provider(self, sec, old_conf):
        """Пытается угадать провайдера по эндпоинту или имени из старого конфига."""
    ...
    def _migrate_if_needed(self):
        """Бесшовная миграция старого монолитного api_keys.ini в новую реляционную структуру."""
            ...
            for sec in old_conf.sections():
                ...
                for k, v in old_conf.items(sec):
                    ...
                    if k in PROVIDER_KEYS:
                        # Записываем ключ провайдеру, только если он еще не задан
    ...
    def get_val(self, service_id, key, default=""):
        ...
        # Сквозное наследование: если ключ принадлежит провайдеру, извлекаем его из providers.ini
    ...
    def set_val(self, service_id, key, value):
        ...
        # Если устанавливается ключ провайдера, он автоматически применяется ко всем моделям агрегатора
        if key in PROVIDER_KEYS:
            if self.models.has_section(sec) and self.models.has_option(sec, "provider"):
                provider = self.models.get(sec, "provider").lower().strip()
            else:
