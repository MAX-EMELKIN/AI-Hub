...
def setup_config(api_config, slug, model_str):
    existing_key = (
        ...
        api_config.get_val("qwen_turbo", "api_key", "") or
...
Сервис: {SERVICE_NAME} через универсальный OpenAI-совместимый API.
\"\"\"
import os
import sys
import time
...
def create_socks5_socket(proxy_host, proxy_port, dest_host, dest_port, timeout=30.0):
    ...
    if resp_header[0] != 5 or resp_header[1] != 0:
        s.close()
        raise ConnectionError(f"SOCKS5: Ошибка соединения (код: {resp_header[1]})")
    atyp = resp_header[3]
    if atyp == 1:
        _recv_all(s, 6)
    elif atyp == 3:
...
class CustomService(BaseService):
    ...
    def _execute_request_raw(self, url, payload_bytes, headers, timeout=60.0):
        ...
        if conn_mode in ("proxy", "socks5"):
            ...
            doh_url = None
            for p_key, p_val in UNIVERSAL_DOH_PRESETS.items():
