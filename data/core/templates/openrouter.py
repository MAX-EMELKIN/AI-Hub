...
def setup_config(api_config, slug, model_str):
    existing_key = (
        api_config.get_val("openrouter_nemotron_3_5", "api_key", "") or
        api_config.get_val("nvidia_3_super_120b", "api_key", "") or
        api_config.get_val("openrouter", "api_key", "") or
        api_config.get_provider_val(PROVIDER_KEY, "api_key", "")
    )
...
Сервис: {SERVICE_NAME} на базе OpenRouter.ai.
\"\"\"
import os
import sys
import time
...
_openrouter_dns_cache = {}
_openrouter_dns_lock = threading.Lock()
...
def resolve_doh(hostname, doh_url):
    with _openrouter_dns_lock:
        if hostname in _openrouter_dns_cache:
            return _openrouter_dns_cache[hostname]
    ...
    try:
        ...
        with urllib.request.urlopen(req, context=ctx, timeout=3.5) as resp:
            if resp.status == 200:
                ...
                if ip:
                    with _openrouter_dns_lock:
                        _openrouter_dns_cache[hostname] = ip
                    return ip
    except Exception:
        pass
...
def create_socks5_socket(proxy_host, proxy_port, dest_host, dest_port, timeout=30.0):
    ...
    if atyp == 1:
        ...
        s.close()
        raise ConnectionError("SOCKS5: Неподдерживаемый тип адреса.")
    return s
...
class CustomService(BaseService):
    ...
    def is_ready(self):
        ...
        if not api_key:
            return False, "API-ключ OpenRouter не заполнен"
        return True, "Сервис {SERVICE_NAME} готов к работе"
    ...
    def _execute_request_raw(self, url, payload_bytes, headers, timeout=60.0):
        ...
        dest_host = parsed.hostname or "openrouter.ai"
        ...
        if conn_mode in ("proxy", "socks5"):
            ...
            doh_url = None
            for p_key, p_val in OPENROUTER_DOH_PRESETS.items():
    ...
    def translate(self, text, src_lang="auto", trg_lang="ru", preset=None):
        ...
        endpoint = self.get_config_val("endpoint", "https://openrouter.ai/api/v1/chat/completions")
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/MAX-EMELKIN/AI-Hub",
            "X-Title": "QTranslate AI Hub",
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
        }
        ...
        try:
            ...
            if status_code >= 400:
                err_msg = data.get("error", {}).get("message", raw_str[:200])
                return f"[OpenRouter HTTP {status_code}]: {err_msg}"

            if data.get("error"):
                err_msg = data["error"].get("message", str(data["error"]))
                return f"[OpenRouter Ошибка: {err_msg}]"
