    ...
    def do_POST(self):
        try:
            ...
            if handler:
                ...
                err_msg = f"[AI Hub]: Сервис '{route}' не найден или отключен в настройках."
            ...
            err_bytes = json.dumps({"response": f"Ошибка обработки запроса: {e}", "success": False}, ensure_ascii=False).encode('utf-8')
            try:
...
def start_server(port=8080, block=False):
    ...
    print(f"[HTTP Server]: Шлюз успешно запущен на http://127.0.0.1:{port}")
