# -*- coding: utf-8 -*-
"""
Модуль: data/core/server.py
Назначение: Локальный HTTP-сервер для QTranslate, диспетчеризация запросов по сервисам,
            поддержка CORS, preflight OPTIONS и логирование входящих вызовов QTranslate.
...
def register_service_route(route_name, handler_func):
    """Регистрирует функцию-обработчик для конкретного сервиса."""
    clean_route = route_name.lower().strip("/")
    SERVICE_DISPATCHER[clean_route] = handler_func

def get_service_handler(route_name):
    """Возвращает обработчик по имени пути или дефолтный активный сервис."""
    clean_route = route_name.lower().strip("/")
...
class HubRequestHandler(http.server.BaseHTTPRequestHandler):
    """Обработчик HTTP-запросов от QTranslate и графического интерфейса."""
    ...
    def do_OPTIONS(self):
        """Мгновенный ответ на пинг готовности от QTranslate."""
    ...
    def do_GET(self):
        """Обработка GET-запросов (статус сервера или аудио-потоки TTS)."""
    ...
    def do_POST(self):
        """Основной обработчик перевода и OCR с логированием."""
        try:
...
class ThreadedHTTPServer(socketserver.ThreadingMixIn, http.server.HTTPServer):
    """Многопоточный сервер с быстрым освобождением сокета."""
