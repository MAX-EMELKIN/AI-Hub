# -*- coding: utf-8 -*-
"""
Модуль: data/core/i18n.py
Назначение: Динамический движок локализации: авто-сканирование data/locales/*.json,
            поддержка любых языков (RU, EN, DE, FR, ZH, CS, TR, ES...), CJK/диакритики и переменных.
...
# Карта стандартных языковых кодов ISO -> Родное отображаемое имя
KNOWN_LANG_NAMES = {
    "ru": "Русский",
    "en": "English",
    "de": "Deutsch",
...
class I18nManager:
    ...
    def _ensure_locale_files(self):
        """Создает дефолтные файлы RU, EN, DE, FR при первом старте."""
        try:
    ...
    def get_available_languages(self):
        """
        Динамически сканирует папку data/locales/*.json и находит ВСЕ языки (RU, EN, DE, FR, ZH, CS, TR...).
        Возвращает список кортежей: [('auto', 'Авто (Windows)'), ('ru', 'Русский'), ('cs', 'Čeština'), ...]
        """
        ...
        for code in sorted(discovered):
            # Пытаемся считать красивое имя из самого файла JSON
            native_name = ""
    ...
    def detect_windows_language(self):
        """Определяет язык системы через WinAPI GetUserDefaultUILanguage."""
        try:
    ...
    def reload(self, lang_override=None):
        ...
        # Двухуровневый fallback: загруженный язык -> английский -> русский
    ...
    def t(self, key, default="", **kwargs):
        """Возвращает перевод фразы с безопасной подстановкой переменных."""
