# -*- coding: utf-8 -*-
"""
Модуль: data/services/webtran/service.py
Назначение: WebTran через защищенный Google PA translateHtml с сохранением переносов строк ZWNJ
            и полной интеграцией с модулем отладки (logger).
...
class WebTranService(BaseService):
    def __init__(self):
        super().__init__(
            service_id="webtran",
            name="WebTran",
    ...
    def is_ready(self):
        return True, "Работает через WebTran / Google PA API (без ключей)"
    ...
    def translate(self, text, src_lang="auto", trg_lang="ru", preset=None):
        ...
        # Защита переносов строк спецсимволом ZWNJ (из скрипта WebTran)
        ...
        headers = {
            ...
            "Origin": "https://www.webtran.eu",
            "Referer": "https://www.webtran.eu/",
        ...
        try:
            ...
            if data and isinstance(data, list) and len(data) > 0 and data[0] and data[0][0]:
                raw_html = data[0][0]
                # Восстановление переносов строк и очистка от ZWNJ
            ...
            if fast: return fast
            return f"Ошибка WebTran: HTTP {he.code}"
            ...
            fast = self.fetch_fast_word(text, src=src_lang, trg=trg_lang)
            if fast: return fast
            print(f"[❌ WebTran Error]: {e}")
            return f"Ошибка WebTran: {e}"

service = WebTranService()