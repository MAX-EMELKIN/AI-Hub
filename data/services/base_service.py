# -*- coding: utf-8 -*-
"""
Модуль: data/services/base_service.py
Назначение: Базовый класс сервисов с фильтрацией кавычек, защитой структуры строк,
            прямым API и интеграцией с модулем логирования logger.
...
class BaseService:
    ...
    def clean_response(self, raw_text, ui_junk_list=None):
        """Глубокая очистка ответа от кавычек, маркеров окончания и мусора с логированием."""
        if not raw_text:
            return ""
        cleaned = raw_text.strip()

        # 1. Удаление тегов подсказок глоссария
        cleaned = re.sub(r'\[\[__GLOSS:[^\]]+__\]\]', '', cleaned)
        cleaned = re.sub(r'\[\s*\[\s*__GLOSS:[^\]]+__\s*\]\s*\]', '', cleaned)

        # 2. Вырезание по маркерам окончания
        for marker in [self.end_marker, "### END ###", "###END", "[[END]]", "[END]"]:
            if marker in cleaned:
                cleaned = cleaned.split(marker)[0].strip()
                break

        # 3. Чистка тройных кавычек
        if '"""' in cleaned:
        ...
        # 5. Удаление служебных строк интерфейса
        if ui_junk_list:
            lines = cleaned.split("\n")
            pure = []
            for line in lines:
