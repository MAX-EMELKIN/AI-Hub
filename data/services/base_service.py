...
LANG_NAMES = {
    ...
    "zh-tw": "китайский язык (традиционный)", "ja": "японский язык", "ko": "корейский язык",
...
class BaseService:
    ...
    def is_ready(self):
        ...
        if missing:
            return False, "Не заполнены обязательные параметры: (" + ", ".join(missing) + ")"
        return True, "Сервис готов к работе"
    ...
    def prepare_text_and_prompt(self, source_text, src_lang="auto", trg_lang="ru", preset=None):
        ...
        prompt_rules = re.sub(
            r'\{TARGET_LANG\}|\{target_lang\}|\{ЦЕЛЕВОЙ_ЯЗЫК\}|\{целевой_язык\}',
            target_name,
            preset_raw,
            flags=re.IGNORECASE
        )
        if trg_lang.lower() != "ru":
            ...
            prompt_rules = re.sub(r'русский язык', f'{target_name}', prompt_rules, flags=re.IGNORECASE)
        ...
        system_prompt = (
            ...
            f"1. Выполни точный и качественный перевод{src_info} на {target_name} с сохранением исходного стиля и смысла.\n"
            f"2. Сохраняй структуру абзацев, отступы и оригинальную пунктуацию.\n"
            f"3. Выводи ИСКЛЮЧИТЕЛЬНО готовый перевод! Запрещено добавлять приветствия, пояснения, сноски и комментарии.\n"
            f"4. Не повторяй текст оригинала, если это не имена собственные или код."
        )
    ...
    def build_prompt_query(self, source_text, src_lang="auto", trg_lang="ru", preset=None):
        ...
        query = (
            f"{system_prompt}\n\n"
            f"5. Обязательно заверши перевод маркером {self.end_marker}. Сам маркер {self.end_marker} оставь в конце без изменений!\n"
            f"6. Исходный текст для перевода:\n"
        ...
        if ui_junk_list:
            ...
            for line in lines:
                ...
                if any(junk.lower() == l.lower() or l.lower().startswith(junk.lower()) for junk in ui_junk_list):
                    continue
                if l.lower().startswith("перевод:") or l.lower().startswith("translation:") or l.startswith('"""') or l.endswith('"""'):
                    continue
                pure.append(line)
            cleaned = "\n".join(pure).strip()

        cleaned = re.sub(
            r'^(?:Вот перевод:|Here is the translation:|Перевод:|Translation:|Текст перевода:|AI Overview:|Обзор от ИИ:)\s*\n*',
            '',
            cleaned,
            flags=re.IGNORECASE
        ).strip()
    ...
    for item in os.listdir(services_root):
        ...
        if os.path.isdir(item_path) and os.path.exists(service_file):
            try:
                ...
                print(f"[Plugin System Error]: [ERROR] Не удалось загрузить сервис '{item}': {e}")