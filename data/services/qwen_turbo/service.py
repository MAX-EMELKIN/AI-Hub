...
class QwenTurboService(BaseService):
    def __init__(self):
        super().__init__(
            service_id="qwen_turbo",
            name="Qwen Turbo (Alibaba DashScope)",
            route_name="/qwen_turbo",
    ...
    def translate(self, text, src_lang="auto", trg_lang="ru", preset=None):
        ...
        model = self.get_config_val("model", "qwen-turbo")
        ...
        try:
            ...
            return f"Ошибка Qwen Turbo: {e}"

service = QwenTurboService()

try:
    from data.core.server import register_service_route
    register_service_route("qwen_turbo", service.translate)
    register_service_route("/qwen_turbo", service.translate)
except Exception:
    pass
