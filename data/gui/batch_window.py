...
class BatchWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        f_box = tk.LabelFrame(pad, text=t("batch_box_files", "Text"), font=theme.font(0, "bold"), bg=bg_card, fg=fg_pri, padx=8, pady=8)
        f_box.pack(fill=tk.X, pady=(0, 10))

        r1 = tk.Frame(f_box, bg=bg_card)
        r1.pack(fill=tk.X, pady=2)
        tk.Label(r1, text=t("batch_src_file", "Text"), width=14, anchor="w", bg=bg_card, fg=fg_pri, font=theme.font(0)).pack(side=tk.LEFT)
        ...
        tk.Button(r1, text=t("btn_browse", "Text"), font=theme.font(-1), bg=theme.get_color("btn_bg"), fg=fg_pri, command=self._browse_input).pack(side=tk.RIGHT)

        r2 = tk.Frame(f_box, bg=bg_card)
        r2.pack(fill=tk.X, pady=2)
        tk.Label(r2, text=t("batch_out_file", "Text"), width=14, anchor="w", bg=bg_card, fg=fg_pri, font=theme.font(0)).pack(side=tk.LEFT)
        ...
        tk.Button(r2, text=t("btn_browse", "Text"), font=theme.font(-1), bg=theme.get_color("btn_bg"), fg=fg_pri, command=self._browse_output).pack(side=tk.RIGHT)

        opt_box = tk.LabelFrame(pad, text=t("batch_box_params", "Text"), font=theme.font(0, "bold"), bg=bg_card, fg=fg_pri, padx=8, pady=8)
        opt_box.pack(fill=tk.X, pady=(0, 10))

        r3 = tk.Frame(opt_box, bg=bg_card)
        r3.pack(fill=tk.X, pady=2)

        tk.Label(r3, text=t("batch_service", "Text"), bg=bg_card, fg=fg_pri, font=theme.font(0)).pack(side=tk.LEFT)
        ...
        tk.Label(r3, text=t("batch_lang", "Text"), bg=bg_card, fg=fg_pri, font=theme.font(0)).pack(side=tk.LEFT)
        ...
        tk.Checkbutton(r3, text=t("batch_chk_protect", "Text"), variable=self.prot_var, bg=bg_card, fg=fg_pri, selectcolor=theme.get_color("input_bg"), font=theme.font(0)).pack(side=tk.LEFT)

        prog_box = tk.LabelFrame(pad, text=t("batch_box_progress", "Text"), font=theme.font(0, "bold"), bg=bg_card, fg=fg_pri, padx=8, pady=8)
        ...
        self.lbl_status = tk.Label(prog_box, text=t("batch_status_ready", "Text"), font=theme.font(-1), fg=theme.get_color("fg_muted"), bg=bg_card, anchor="w")
        ...
        self.btn_start = tk.Button(
            btn_bar, text=t("batch_btn_start", "Text"), font=theme.font(0, "bold"),
        ...
        self.btn_pause = tk.Button(
            btn_bar, text=t("batch_btn_pause", "Text"), font=theme.font(0), relief=tk.FLAT,
        ...
        self.btn_cancel = tk.Button(
            btn_bar, text=t("batch_btn_cancel", "Text"), font=theme.font(0), relief=tk.FLAT,
    ...
    def _start(self):
        ...
        if not in_f or not os.path.exists(in_f):
            messagebox.showwarning(t("btn_settings", "Text"), t("batch_warn_select_file", "Text"), parent=self)
            return

        self.btn_start.config(state="disabled")
        self.btn_pause.config(state="normal", text=t("batch_btn_pause", "Text"))
    ...
    def _update_ui_progress(self, curr, total, pct, prev):
        self.pbar["value"] = pct
        status_str = t("batch_status_translating", f"Text", curr=curr, total=total, pct=pct)
        self.lbl_status.config(text=status_str)
        preview_str = t("batch_preview_prefix", f"Text", prev=prev)
        self.preview_lbl.config(text=preview_str)

    def _toggle_pause(self):
        pause_label = t("batch_btn_pause", "Text")
        resume_label = t("batch_btn_resume", "Text")
        if self.btn_pause["text"] == pause_label:
            batch_pipeline.pause()
            self.btn_pause.config(text=resume_label)
            self.lbl_status.config(text=t("batch_status_paused", "Text"))
        else:
    ...
    def _cancel(self):
        ...
        self.lbl_status.config(text=t("batch_status_cancelled", "Text"))

    def _on_finish(self, out_path):
        self.pbar["value"] = 100
        self.lbl_status.config(text=t("batch_status_done", "Text"))
        self.btn_start.config(state="normal")
        self.btn_pause.config(state="disabled")
        self.btn_cancel.config(state="disabled")

        done_msg = t("batch_done_msg", f"Text", path=out_path)
        messagebox.showinfo("OK", done_msg, parent=self)

    def _on_error(self, err_msg):
        err_str = t("batch_status_error", f"Text", err=err_msg)
        self.lbl_status.config(text=err_str)
        self.btn_start.config(state="normal")
        self.btn_pause.config(state="disabled")
        self.btn_cancel.config(state="disabled")
        messagebox.showerror(t("status_error", "Text"), err_msg, parent=self)
