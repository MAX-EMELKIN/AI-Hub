...
class BatchWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        f_box = tk.LabelFrame(
            pad, text=t("batch_box_files", "Файлы для перевода"),
            font=theme.font(0, "bold"), bg=bg_card, fg=fg_pri, padx=8, pady=8
        )
        f_box.pack(fill=tk.X, pady=(0, 10))

        r1 = tk.Frame(f_box, bg=bg_card)
        r1.pack(fill=tk.X, pady=2)
        tk.Label(r1, text=t("batch_src_file", "Исходный файл:"), width=16, anchor="w", bg=bg_card, fg=fg_pri, font=theme.font(0)).pack(side=tk.LEFT)
        ...
        tk.Label(r2, text=t("batch_out_file", "Выходной файл:"), width=16, anchor="w", bg=bg_card, fg=fg_pri, font=theme.font(0)).pack(side=tk.LEFT)
        ...
        opt_box = tk.LabelFrame(
            pad, text=t("batch_box_params", "Параметры перевода"),
            font=theme.font(0, "bold"), bg=bg_card, fg=fg_pri, padx=8, pady=8
        )
        ...
        tk.Checkbutton(
            r3, text=t("batch_chk_protect", "Защита программного кода"),
            variable=self.prot_var, bg=bg_card, fg=fg_pri, selectcolor=theme.get_color("input_bg"), font=theme.font(0)
        ).pack(side=tk.LEFT)

        prog_box = tk.LabelFrame(
            pad, text=t("batch_box_progress", "Прогресс выполнения"),
            font=theme.font(0, "bold"), bg=bg_card, fg=fg_pri, padx=8, pady=8
        )
    ...
    def _browse_input(self):
        fn = filedialog.askopenfilename(
            filetypes=[("Текстовые файлы", "*.txt;*.sub;*.srt;*.log;*.json;*.xml"), ("Все файлы", "*.*")],
            parent=self
        )
        if fn:
    ...
    def _start(self):
        ...
        if not in_f or not os.path.exists(in_f):
            messagebox.showwarning(
                ...
                t("batch_warn_select_file", "Пожалуйста, выберите существующий входной файл для перевода."),
                parent=self
            )
            return
    ...
    def _update_ui_progress(self, curr, total, pct, prev):
        self.pbar["value"] = pct
        status_str = t("batch_status_translating", "Перевод фрагментов: {curr} из {total} ({pct}%)", curr=curr, total=total, pct=pct)
        self.lbl_status.config(text=status_str)
        preview_str = t("batch_preview_prefix", "Текущий фрагмент: {prev}", prev=prev)
    ...
    def _toggle_pause(self):
        ...
        if self.btn_pause["text"] == pause_label:
            ...
            self.lbl_status.config(text=t("batch_status_paused", "Перевод приостановлен"))
        else:
    ...
    def _cancel(self):
        ...
        self.lbl_status.config(text=t("batch_status_cancelled", "Перевод отменен"))
    ...
    def _on_finish(self, out_path):
        ...
        done_msg = t("batch_done_msg", "Документ успешно переведен и сохранен по пути:\n{path}", path=out_path)
        messagebox.showinfo("OK", done_msg, parent=self)

    def _on_error(self, err_msg):
        err_str = t("batch_status_error", "Ошибка перевода: {err}", err=err_msg)
