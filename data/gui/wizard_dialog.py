...
Назначение: Студия подключения сервисов (AddServiceWizardDialog).
            Динамически считывает модульные шаблоны из папки data/core/templates/
            (Cerebras, SiliconFlow, Mistral, Pollinations, Boltch, OpenRouter,
            DashScope, DeepSeek, Custom и др.), автоматически подтягивает сохраненные
            ключи и прокси из providers.ini, не закрывает окно после создания,
            содержит встроенный тестовый стенд (быстрый и полный пинг) и поддерживает
            умное открытие файлов в Notepad++ (при наличии в системе) или во встроенном редакторе.
...
def open_file_in_smart_editor(file_path, parent_window=None):
    """Открывает файл в Notepad++ (если он есть в системе) или во встроенном окне Хаба."""
    ...
    # Автопоиск установленных продвинутых редакторов
    candidates = [
    ...
    # Если стороннего редактора нет — открываем встроенный редактор Хаба
    CodeEditorDialog(parent_window, file_path)


class CodeEditorDialog(tk.Toplevel):
    """Автономный встроенный редактор кода с подсветкой и возможностью сохранения."""
...
class AddServiceWizardDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        # 7. Сетевой режим (Direct / SOCKS5 / DoH)
        ...
        # 8. Политика размышлений (Thinking)
        ...
        # 9. Системный ID кнопки в QTranslate
        ...
        # ПАНЕЛЬ ДЕЙСТВИЙ И ТЕСТИРОВАНИЯ (ПОЯВЛЯЕТСЯ ПОСЛЕ СОЗДАНИЯ)
    ...
    def _on_provider_change(self, event=None):
        ...
        if p_key != "custom":
            ...
            # Автоподстановка сохраненного ключа и прокси из providers.ini
