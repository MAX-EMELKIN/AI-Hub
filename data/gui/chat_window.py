...
class UniversalChatClient:
    def __init__(self, service_id):
        ...
        self.is_gemini = (
            service_id == "gemini_family" or
            "gemini" in self.model.lower() or
            "generativelanguage" in api_config.get_val(service_id, "endpoint", "")
        )
        if self.is_gemini:
    ...
    def send_chat(self, history, enable_search):
        sys_prompt = (
            ...
            "Отвечай четко, по существу, соблюдай форматирование Markdown и пиши на языке пользователя."
        )
        if enable_search:
            sys_prompt += (
    ...
    def _send_cloudflare(self, history, sys_prompt, opener):
        ...
        try:
            ...
            elapsed = time.time() - t_call
            logger.api_summary(f"Chat: {self.service_id}", self.model, elapsed, 0, note=f"Exception: {e}")
            return f"Исключение при вызове Cloudflare: {e}"
...
class ChatWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        logger.system("Окно ИИ-чата открыто")
    ...
    def _get_api_models(self):
        ...
        for s_id, s_obj in LOADED_SERVICES.items():
            if not getattr(s_obj, "is_ai_service", True):
                continue
    ...
    def _build_ui(self):
        ...
        top_bar = tk.Frame(
            pad, bg=bg_card, padx=8, pady=6, relief=tk.SOLID, bd=1,
        ...
        chk_search = tk.Checkbutton(
            top_bar, text="Веб-поиск", variable=self.var_search,
            bg=bg_card, fg=fg_pri, selectcolor=in_bg, font=theme.font(0), cursor="hand2"
        )
        chk_search.pack(side=tk.LEFT)
        ToolTip(chk_search, "Разрешить модели вызывать веб-поиск и чтение ссылок для проверки фактов")

        btn_clear = tk.Button(
            top_bar, text="Очистить историю", font=theme.font(-1, "bold"), relief=tk.FLAT,
        ...
        ToolTip(self.btn_send, "Отправить сообщение нейросети (Ctrl+Enter)")
        ...
        self._append_to_chat("sys_text", "Чат готов к общению. Введите ваш вопрос ниже.")
    ...
    def _clear_chat(self):
        ...
        self._append_to_chat("sys_text", "История диалога очищена.")
    ...
    def _bg_worker(self, service_id, enable_search):
        ...
        try:
            ...
            response = f"Ошибка генерации ответа: {e}"

        def _on_finish():
            self._is_generating = False
            self.btn_send.config(state="normal", text="Отправить")
            self.combo_model.config(state="readonly")
            self._append_to_chat("ai_name", f"ИИ ({client.model or service_id}):", is_name=True)
