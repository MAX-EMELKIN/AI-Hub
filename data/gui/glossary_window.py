...
class GlossaryEnrichmentDialog(tk.Toplevel):
    def __init__(self, parent, on_enriched_callback=None):
        ...
        self.title(t("enrich_title", "Мастер морфологического обогащения глоссария"))
    ...
    def _build_ui(self):
        ...
        tk.Label(
            pad, text=t("enrich_header", "Генерация форм множественного числа и склонений через ИИ"),
        ...
        tk.Label(r1, text=t("enrich_model_lbl", "Модель генерации:"), font=theme.font(0, "bold"), fg=fg_pri, width=22, anchor="w", bg=bg_card).pack(side=tk.LEFT)
        ...
        tk.Label(r2, text=t("enrich_src_lbl", "Язык оригинала:"), font=theme.font(0, "bold"), fg=fg_pri, width=16, anchor="w", bg=bg_card).pack(side=tk.LEFT)
        self.combo_src = ttk.Combobox(r2, values=[n for _, n in LANG_OPTIONS], state="readonly", width=16)
        self.combo_src.set("English")
        self.combo_src.pack(side=tk.LEFT, padx=(0, 10))

        tk.Label(r2, text=t("enrich_trg_lbl", "Язык перевода:"), font=theme.font(0, "bold"), fg=fg_pri, width=16, anchor="w", bg=bg_card).pack(side=tk.LEFT)
        self.combo_trg = ttk.Combobox(r2, values=[n for _, n in LANG_OPTIONS], state="readonly", width=16)
        ...
        tk.Checkbutton(
            r3, text=t("enrich_chk_plurals", "Множественное число (Plurals: -s, -es)"),
            variable=self.var_plurals, bg=bg_card, fg=fg_pri, selectcolor=in_bg, font=theme.font(-1)
        ).pack(side=tk.LEFT, padx=(0, 10))
        self.var_spelling = tk.BooleanVar(value=True)
        tk.Checkbutton(
            r3, text=t("enrich_chk_spelling", "Грамматические формы и притяжательные падежи ('s)"),
            variable=self.var_spelling, bg=bg_card, fg=fg_pri, selectcolor=in_bg, font=theme.font(-1)
        ).pack(side=tk.LEFT)
        ...
        self.lbl_status = tk.Label(
            prog_frame, text=t("enrich_status_ready", "Готов к обработке терминов словаря"),
            font=theme.font(-1), fg=theme.get_color("fg_muted"), bg=bg_card, anchor="w"
        )
        self.lbl_status.pack(fill=tk.X)

        tk.Label(pad, text=t("enrich_preview_lbl", "Сгенерированные термины:"), font=theme.font(0, "bold"), fg=fg_pri, bg=bg_main).pack(anchor="w", pady=(0, 2))
        ...
        self.btn_start = tk.Button(
            btn_bar, text=t("enrich_btn_start", "Начать генерацию"), font=theme.font(0, "bold"),
    ...
    def _start_enrichment(self):
        ...
        if not terms:
            messagebox.showwarning(
                ...
                t("enrich_warn_empty_dict", "Глоссарий пуст. Добавьте хотя бы один термин перед обогащением."),
                parent=self
            )
            return
    ...
    def _worker_thread(self, service, terms, src_lang, trg_lang):
        ...
        src_name = service.get_target_lang_name(src_lang) if hasattr(service, "get_target_lang_name") else src_lang
        all_new_terms = {}
        ...
        system_instruction = (
            ...
            f"ALLOWED ONLY:\n"
            f"- Grammatically correct plurals and inflected variants.\n"
    ...
    def _update_progress(self, curr, total, pct, new_count):
        def _ui():
            ...
            status_text = t(
                ...
                "Обработка пакета {curr} из {total} ({pct}%). Найдено новых форм: {new_count}",
    ...
    def _finish_work(self, orig_count, new_count):
        def _ui():
            ...
            if new_count > 0:
                ...
                done_text = t(
                    "enrich_status_done",
                    "Генерация завершена. Исходно: {orig}, Добавлено: {new_count}, Итого: {total}",
                    orig=orig_count, new_count=new_count, total=orig_count + new_count
                )
                self.lbl_status.config(text=done_text)
            else:
                self.lbl_status.config(text=t("enrich_status_empty", "Новых морфологических форм не обнаружено"))
        self.after(0, _ui)

    def _apply_to_glossary(self):
        if not self._generated_terms:
            return
        ...
        success_text = t(
            "enrich_success_msg",
            "Глоссарий успешно дополнен!\nДобавлено новых вариантов: {count}\nВсего терминов: {total}",
...
class BatchGlossaryImportDialog(tk.Toplevel):
    def __init__(self, parent, on_imported_callback=None):
        ...
        bg_main = theme.get_color("bg_main")
        self.title(t("import_title", "Массовый импорт терминов в словарь"))
    ...
    def _build_ui(self):
        ...
        tk.Label(
            pad, text=t("import_header", "Вставьте список терминов или загрузите файл со словарем:"),
            font=theme.font(1, "bold"), fg=theme.get_color("accent"), bg=bg_main
        ...
        self.btn_pre_enrich = tk.Button(
            act_bar, text=t("import_btn_enrich_pre", "Сгенерировать формы через ИИ"), font=theme.font(-1, "bold"),
            relief=tk.FLAT, bg=theme.get_color("accent"), fg=theme.get_color("accent_text"),
            cursor="hand2", padx=8, pady=2, command=self._enrich_list_before_import
        )
        self.btn_pre_enrich.pack(side=tk.LEFT, padx=6)
        ToolTip(self.btn_pre_enrich, t("import_tip_enrich_pre", "Автоматически добавить формы множественного числа к списку перед сохранением"))
        ...
        default_info_text = (
            "Поддерживаемые разделители пар: '=' или '->' или табуляция (TSV).\n"
            "Пример: Pip-Boy = Пип-бой"
        )
        tk.Label(
        ...
        tk.Checkbutton(
            info_frame, text=t("import_chk_merge", "Объединить с текущим словарем"), variable=self.var_merge,
        ...
        tk.Button(
            btn_bar, text=t("import_btn_do_import", "Импортировать в глоссарий"), font=theme.font(0, "bold"),
            relief=tk.FLAT, bg=theme.get_color("accent"), fg=theme.get_color("accent_text"),
    ...
    def _load_file(self):
        fn = filedialog.askopenfilename(
            filetypes=[("Файлы словарей и таблиц", "*.txt;*.tsv;*.csv;*.dic"), ("Все файлы", "*.*")],
            parent=self
        )
    ...
    def _enrich_list_before_import(self):
        ...
        if not raw:
            messagebox.showwarning(
                t("btn_settings", "Внимание"),
                t("import_warn_empty", "Текстовое поле пусто. Вставьте список терминов для обработки."),
                parent=self
            )
            return
        ...
        if not parsed:
            messagebox.showwarning(
                t("btn_settings", "Внимание"),
                t("import_warn_parse_fail", "Не удалось распознать пары терминов. Используйте формат: Термин = Перевод"),
                parent=self
            )
            return
        ...
        if not avail:
            messagebox.showerror(
                ...
                t("enrich_err_no_services", "Нет доступных нейросетевых сервисов для генерации форм."),
                parent=self
            )
            return

        service = avail[0]
        self.btn_pre_enrich.config(state="disabled", text="Генерация...")
        ...
        def _worker():
            ...
            system_instruction = (
                ...
                "ALLOWED ONLY:\n"
                "- Grammatically correct plurals and inflected variants.\n"
            ...
            for batch in batches:
                ...
                try:
                    ...
                    for k, v in gen_parsed.items():
                        ...
                        clean_v = v.strip()
                        if not clean_k or len(clean_k.split()) > 4:
                            continue
            ...
            def _done():
                self.btn_pre_enrich.config(state="normal", text=t("import_btn_enrich_pre", "Сгенерировать формы через ИИ"))
                formatted_lines = [f"{k} = {v}" for k, v in sorted(all_combined.items(), key=lambda x: str(x[0]).lower())]
                self.text_area.delete("1.0", tk.END)
                self.text_area.insert("1.0", "\n".join(formatted_lines))
                done_msg = t(
                    "import_pre_done_msg",
                    "Обработка завершена!\nБыло терминов: {orig}\nДобавлено новых форм: {new_count}\nВсего в списке: {total}",
    ...
    def _do_import(self):
        ...
        if not raw:
            messagebox.showwarning(
                t("btn_settings", "Внимание"),
                t("import_warn_empty", "Текстовое поле пусто."),
                parent=self
            )
            return
        ...
        if not parsed:
            messagebox.showwarning(
                t("btn_settings", "Внимание"),
                t("import_warn_parse_fail", "Не удалось распознать пары терминов."),
                parent=self
            )
            return
        ...
        success_msg = t(
            "import_success_msg",
            "Импорт успешно завершен!\nДобавлено / обновлено терминов: {count}\nВсего терминов в глоссарии: {total}",
...
class GlossaryWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        self.lbl_count = tk.Label(
            top_bar, text=t("dict_total_count", "Всего терминов: {count}", count=0),
            font=theme.font(-1), fg=theme.get_color("fg_muted"), bg=bg_main
        )
        self.lbl_count.pack(side=tk.LEFT)

        btn_enrich_all = tk.Button(
            top_bar, text=t("dict_btn_enrich_all", "Обогатить словарь ИИ"), font=theme.font(-1, "bold"),
            relief=tk.FLAT, bg=theme.get_color("accent"), fg=theme.get_color("accent_text"),
            cursor="hand2", padx=8, pady=2, command=self._open_enrichment_wizard
        )
        btn_enrich_all.pack(side=tk.RIGHT)
        ToolTip(btn_enrich_all, t("dict_tip_enrich_all", "Сгенерировать недостающие формы множественного числа и падежей для всех слов словаря"))

        btn_batch = tk.Button(
            top_bar, text=t("dict_btn_batch_import", "Массовый импорт"), font=theme.font(-1, "bold"),
            relief=tk.FLAT, bg=theme.get_color("help_btn_bg"), fg=theme.get_color("help_btn_fg"),
            cursor="hand2", padx=8, pady=2, command=self._open_batch_import
        )
        btn_batch.pack(side=tk.RIGHT, padx=6)
        ToolTip(btn_batch, t("dict_tip_batch_import", "Загрузить словарь списком из буфера обмена или текстового файла"))
        ...
        self.tree.heading("orig", text=t("dict_col_orig", "Исходный термин (Оригинал)"))
        self.tree.heading("trans", text=t("dict_col_trans", "Перевод в игре / тексте"))
        ...
        ToolTip(self.btn_enrich, t("dict_tip_enrich_single", "Сгенерировать через ИИ множественные и падежные формы для выбранных терминов"))

        self.btn_delete = tk.Button(
            btn_row, text=t("dict_btn_delete", "Удалить термин"), font=theme.font(0),
    ...
    def _refresh_tree_view(self):
        ...
        if q:
            self.lbl_count.config(text=t("dict_found_count", "Найдено: {shown} из {total}", shown=items_shown, total=total_count))
        else:
            self.lbl_count.config(text=t("dict_total_count", "Всего терминов: {count}", count=total_count))
    ...
    def _on_select_item(self, event):
        ...
        if len(sel) == 1:
            ...
            self.btn_delete.config(text=t("dict_btn_delete", "Удалить термин"))
        elif len(sel) > 1:
            self.btn_enrich.config(text=t("dict_btn_enrich_selected", "Обогатить ({count})", count=len(sel)))
            self.btn_delete.config(text=t("dict_btn_delete_selected", "Удалить ({count})", count=len(sel)))
        else:
            self.btn_enrich.config(text=t("dict_btn_enrich_single", "Обогатить термин"))
            self.btn_delete.config(text=t("dict_btn_delete", "Удалить термин"))
    ...
    def _add_term(self):
        ...
        if not k or not v:
            messagebox.showwarning(
                t("btn_settings", "Внимание"),
                t("dict_warn_fill", "Пожалуйста, заполните оба поля: оригинал и перевод."),
                parent=self
            )
            return
    ...
    def _enrich_selected_or_single(self):
        ...
        if not avail:
            messagebox.showerror(
                t("status_error", "Ошибка"),
                t("dict_warn_no_ai", "Нет доступных ИИ-сервисов для выполнения генерации."),
                parent=self
            )
            return
        ...
        self.btn_enrich.config(state="disabled", text=f"Генерация ({total_terms})...")
        self.lbl_count.config(text=t("dict_enrich_generating", "Генерация форм для {count} терминов через {name}...", count=total_terms, name=service.name))
        ...
        def _worker():
            ...
            for batch in batches:
                ...
                try:
                    ...
                    for gen_k, gen_v in parsed.items():
                        ...
                        if not clean_k or len(clean_k.split()) > 4:
                            continue
                        if clean_k.lower() in current_lower:
                            continue
                        current_lower.add(clean_k.lower())
            ...
            def _done():
                ...
                if new_added > 0:
                    self.lbl_count.config(text=f"Добавлено новых форм: +{new_added}")
                    preview = "\n".join(f"- {x}" for x in added_list[:12])
                    if len(added_list) > 12:
                        preview += f"\n... (и еще {len(added_list) - 12})"
                    done_msg = t(
                        "dict_enrich_done_msg",
                        "Обогащение успешно завершено!\nОбработано терминов: {count}\nСгенерировано вариантов: {new_count}\n\nПримеры:\n{preview}",
                        count=total_terms, new_count=new_added, preview=preview
                    )
                    messagebox.showinfo("OK", done_msg, parent=self)
                else:
                    self.lbl_count.config(text=t("dict_enrich_none_msg", "Новых форм не найдено"))
                    messagebox.showinfo("OK", t("dict_enrich_none_msg", "Новых форм не найдено"), parent=self)
    ...
    def _delete_selected_terms(self):
        ...
        if count == 1:
            item = self.tree.item(sel[0])
            k = item["values"][0]
            del_msg = t("dict_delete_confirm_single", "Удалить термин '{name}' из словаря?", name=k)
            if messagebox.askyesno(t("confirm_delete_title", "Подтверждение удаления"), del_msg, parent=self):
                terms = glossary_engine.get_all_terms()
                if k in terms:
                    del terms[k]
                    glossary_engine.save_all_terms(terms)
            ...
            del_multi_msg = t("dict_delete_confirm_multi", "Удалить выбранные термины ({count} шт.) из словаря?", count=count)
            if messagebox.askyesno(t("confirm_delete_title", "Подтверждение удаления"), del_multi_msg, parent=self):
                terms = glossary_engine.get_all_terms()
                for s_item in sel:
                    item_data = self.tree.item(s_item)
