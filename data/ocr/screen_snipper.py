# -*- coding: utf-8 -*-
"""
Модуль: data/ocr/screen_snipper.py
Назначение: Высокоточный захват области экрана (BitBlt) с авто-масштабированием мелкого текста (Upscale),
            добавлением белых полей (Padding) и генерацией 24-bit BMP для нейросетевого OCR.
...
# Настройка StretchBlt для качественного масштабирования без размытия
...
def capture_exact_screen_box(target_path, x, y, w, h, base_pad=24):
    """
    Вырезает область экрана, автоматически увеличивает мелкий текст (x2/x3)
    и обрамляет белыми полями для безошибочной детекции нейросетью MNN.
    """
    if w <= 0 or h <= 0:
        return False

    # 1. Автоматический коэффициент масштабирования для мелкого текста
    scale = 1
    if h < 45 or w < 120:
        scale = 3
    elif h < 80 or w < 200:
    ...
    # 2. Заливаем всё поле чистым белым цветом
    gdi32.PatBlt(hdc_mem, 0, 0, dst_w, dst_h, WHITENESS)

    # 3. Высококачественное масштабирование через StretchBlt (HALFTONE)
    if scale > 1:
    ...
    # 4. Считываем пиксели в буфер 24-bit BMP
...
class ScreenSnipper:
    """Экранный оверлей со стоп-кадром и точным захватом координат."""
    ...
    def _on_press(self, event):
        ...
        self.cur_rect = self.canvas.create_rectangle(
            event.x, event.y, event.x, event.y, outline="#00e5ff", width=2, fill="#ffffff"
        )
    ...
    def _on_release(self, event):
        ...
        # Скрываем оверлей ДО вырезания области экрана
